...
export class Shadow_Textured_Phong_NM_Shader extends defs.Phong_Shader {
    shared_glsl_code() {
        // ********* SHARED CODE, INCLUDED IN BOTH SHADERS *********
        return ` precision mediump float;
                const int N_LIGHTS = ` + this.num_lights + `;
    ...
    fragment_glsl_code() {
        ...
        return this.shared_glsl_code() + `
                ...
                void main(){
                    ...
                    // Compute the final color with contributions from lights:
                    vec3 diffuse, specular;
                    vec3 other_than_ambient = phong_model_lights( normal_tex, vertex_worldspace, diffuse, specular );
                    //vec3 other_than_ambient = phong_model_lights( normalize(N), vertex_worldspace, diffuse, specular );
    ...
    update_GPU(context, gpu_addresses, gpu_state, model_transform, material) {
        ...
        if (material.normal && material.normal.ready && material.color_texture && material.color_texture.ready) {
            context.uniform1i(gpu_addresses.normal, 0);
            material.normal.activate(context);
            context.uniform1i(gpu_addresses.color_texture, 1);
            material.color_texture.activate(context, 1); // :)
        }
